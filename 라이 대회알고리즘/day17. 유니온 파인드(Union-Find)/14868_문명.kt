...
private val direction = arrayOf(Point(1, 0), Point(-1, 0), Point(0, 1), Point(0, -1))

private fun find(p: IntArray, n: Int): Int {
    if (p[n] < 0) {
        return n
    }
    p[n] = find(p, p[n])
...
fun main() {
    ...
    val visit = Array(N + 1) { IntArray(N + 1) { -1 } }
    val p = IntArray(K + 1) { -1 }

    for (i in 1..K) {
        val (x, y) = readlnInts()
        visit[x][y] = i
        q.add(intArrayOf(x, y, i))
    }
    ...
    for (i in 0 until q.size) {
        ...
        for ((dx, dy) in direction) {
            ...
            if (1 <= nx && nx <= N && 1 <= ny && ny <= N) {
                if (visit[nx][ny] != -1 && visit[nx][ny] != idx) {
                    union(p, idx, visit[nx][ny])
                    if (p[1] == -K) {
                        println(level)
                        return
                    }
    ...
    while (true) {
        ...
        val qSize = q.size
        repeat(qSize) {
            val (x, y, idx) = q.removeFirst()
            for ((dx, dy) in direction) {
                val (nx, ny) = (x + dx to y + dy)
                if (1 <= nx && nx <= N && 1 <= ny && ny <= N) {
                    if (visit[nx][ny] == -1) {
                        visit[nx][ny] = idx
                        q.add(intArrayOf(nx, ny, idx))
                        for ((dx2, dy2) in direction) {
                            val (nx2, ny2) = (nx + dx2 to ny + dy2)
                            if (1 <= nx2 && nx2 <= N && 1 <= ny2 && ny2 <= N) {
                                if (visit[nx2][ny2] != -1 && visit[nx2][ny2] != idx) {
                                    union(p, idx, visit[nx2][ny2])
                                    if (p[1] == -K) {
                                        print(level)
                                        return
                                    }
