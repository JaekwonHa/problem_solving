...
def merge(A, B):
    count_0, count_1, count_2 = A.count(B[0]), A.count(B[1]), A.count(B[2])

    if count_0 + count_1 + count_2 == 3:
        return A
    elif count_0 + count_1 + count_2 == 2:
        if count_0 + count_1 == 2:
            if B[1] == A[-1]:
                A.append(B[-1])
                return A
        elif count_0 + count_2 == 2:
            index_A, index_C = A.index(B[0]), A.index(B[2])
            if index_A + 1 == index_C:
                return A[:index_A] + B + A[index_C + 1:]
        elif count_1 + count_2 == 2:
            if B[1] == A[0]:
                return [B[0]] + A
    elif count_0 + count_1 + count_2 == 1:
        if count_0 == 1:
            if B[0] == A[-1]:
                return A + B[1:]
        elif count_2 == 1:
...
def recoverSecret(triplets):
    ...
    for triple in triplets:
        answer = triple
        candidate = triplets.copy()
        index = 0
        while 0 < len(candidate) and index < len(candidate):
            temp = answer.copy()
            try:
                answer = merge(answer, candidate[index])
                del candidate[index]
                index = 0
            except MyError as e:
                index += 1
...
def permutations(string):
    return list(set(map(''.join, itertools.permutations(list(string)))))