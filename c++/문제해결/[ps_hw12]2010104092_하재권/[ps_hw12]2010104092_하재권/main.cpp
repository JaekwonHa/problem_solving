...
int xxx;
std::vector<std::pair<double, double> > yyy;
...
public:
  ...
  explicit ring_incident_edge_iterator(const ring_graph& g,
    ...
    // ring_incident_edge_iterator::iterator_adaptor_(g.n() > 2 ? 2:1), m_n(g.n()),m_u(u) {};
	ring_incident_edge_iterator::iterator_adaptor_(4), m_n(g.n()),m_u(u) {};

private:
  friend class boost::iterator_core_access;

  edge_descriptor dereference() const {
    //static const int ring_offset[] = {1, -1 , 2};
	  static int *ring_offset = new int[xxx];
	  for(int i=1; i<=xxx; i++) ring_offset[i-1]=i;
    vertex_descriptor v;
...
std::ostream& operator<<(std::ostream& output, const edge_descriptor& e) {
	return output << "(" << yyy[e.first].first <<"."<<yyy[e.first].second<<") (" << yyy[e.second].first <<"."<<yyy[e.second].second<<")";
}
...
int main (int argc, char const *argv[]) {
	...
	for(int c=0; c<cases; c++) {
	  ...
	  if(c!=0) std::cout<<std::endl;
		...
		for(int i=0; i<xxx; i++) {
			std::cin>>x>>y;
			yyy.push_back(std::make_pair(x,y));
		}
  ...
  // of 5.
  std::size_t n = argc == 2 ? boost::lexical_cast<std::size_t>(argv[1]) : xxx;
  ...
  if (n>0) {
    ...
    dijkstra_shortest_paths(g, source, predecessor_map(&pred[0]). distance_map(&dist[0]) );
  ...
  */
  vertex_descriptor source = 0;
	...
	std::cout<<"frekles min cost : "<<cost<<std::endl;
	}
  return 0;
}