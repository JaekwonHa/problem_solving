...
int string_compare(char *s, char *t) {
	...
	for(i=1; i<strlen(s); i++)
		for(j=1; j<strlen(t); j++) {
			...
			opt[INSERT] = m[i][j-1].cost + ins(t[j]);
...
int match(char c, char d) {	// 교체를 하기 위해서는 delete + insert 해야 하기에 cost가 3이 됩니다.
	if(c==d) return 0;
	else return 3;
}
int ins(char c) {			// insert는 cost 1
	return 1;
}
int del(char c) {			// delete는 cost 2
	return 2;
}
void goal_cell(char *s, char *t, int *i, int *j) {
	*i = strlen(s)-1;
...
void match_out(char *s, char *t, int i, int j) {
	if(s[i]==t[j]) cout<<"M";
	else cout<<"DI";		// 교체를 허용 하지 않았기 때문에 표시를 DI 로 표시한다.
}